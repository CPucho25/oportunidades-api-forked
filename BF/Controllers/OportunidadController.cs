...
namespace BF.Controllers
{
    ...
    public class OportunidadController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("ValidarEstado")]
        public RespondSearchObject<DTO.DTOLista> validarEstado(DTO.DTOOportunidad dto)
        {
            var oportunidad = _context.OpMOportunidad.First(elemento => elemento.FlgActivo == 1 && elemento.Id == dto.Id).Fee;
            var importes = _context.OpMPrograFacturacion.Where(importe => importe.FlgActivo == 1 && importe.id_oportunidad == dto.Id).Select(importe => importe.monto).Sum();
            ...
            if (oportunidad == importes && oportunidad != 0)
            {
                return new RespondSearchObject<DTO.DTOLista>()
                {
                    Mensaje = "Se puede actualizar",
                    Flag = true
                };
            }
            else
            {
                if (oportunidad > importes)
                {
                    return new RespondSearchObject<DTO.DTOLista>()
                    {
                        Mensaje = string.Format("Tiene que completar la programación de facturación para cambiar el estado a  Aprobado o Renovado, tiene un saldo {0} por programar facturación", oportunidad-importes),
                        Flag = false
                    };
                }
                else
                {
                    return new RespondSearchObject<DTO.DTOLista>()
                    {
                        Mensaje = string.Format("Tiene que completar la programación de facturación para cambiar el estado a  Aprobado o Renovado, tiene un exceso de {0} con respecto al fee de la oportunidad", importes-oportunidad),
                        Flag = false
                    };
                }
            }
        ...
        [HttpPost]
        [Route("validarSaldoFee")]
        public RespondSearchObject<DTO.DTOValSaldoFee>  validarSaldoFee(DTOValSaldoFee validacion)
        {
            var oportunidad = _context.OpMOportunidad.First(elemento => elemento.FlgActivo == 1 && elemento.Id == validacion.id).Fee;
            var importes = _context.OpMPrograFacturacion.Where(importe => importe.FlgActivo == 1 && importe.id_oportunidad == validacion.id).Select(importe => importe.monto).Sum();
            ...
            if (oportunidad != 0)
            {
                switch(validacion.tipo)
                {
                    ...
                    // tipo 1 -> Modificar
                    case 0:
                        if(oportunidad >= importes + validacion.valorActual)
                        {
                            return new RespondSearchObject<DTO.DTOValSaldoFee>()
                            {
                                Objeto = validacion,
                                Mensaje = "Se Actualizo los datos",
                                Flag = true
                        ...
                        else
                        {
                            ...
                            {
                                Objeto = validacion,
                                Mensaje = string.Format("El importe máximo a ingresar es de {0}", (oportunidad - importes).ToString()),
                                Flag = false
                            };
                        }

                    case 1:
                        if (oportunidad >= importes + validacion.valorActual - validacion.valorAnterior)
                        {
                            return new RespondSearchObject<DTO.DTOValSaldoFee>()
                            {
                                Objeto = validacion,
                        ...
                        else
                        {
                            ...
                            {
                                Objeto = validacion,
                                Mensaje = string.Format("El importe máximo a ingresar es de {0}", oportunidad + validacion.valorAnterior - importes),
                                Flag = false
                            };
                        }
                ...
                switch (dto.IdRol)
                {
                    ...
                    case 4:
                        listOportunidad = await _context.VWOportunidadEA.Where(o => o.FlgActivo == 1 && o.IdEa == dto.IdEa
                                                                                        ...
                                                                                        && (dto.IdPeriodo == 0 ? 1 == 1 : o.IdPeriodo == dto.IdPeriodo)
                                                                                        && (dto.IdGerente == 0 ? 1 == 1 : o.IdGerente == dto.IdGerente)
                                                                                        && (dto.IdSocio == 0 ? 1 == 1 : o.IdSocio == dto.IdSocio)//para que funcione el combo Socio //CP
                                                                                        && (dto.IdEmpresa == 0 ? 1 == 1 : o.IdEmpresa == dto.IdEmpresa)
                                                                                        && (dto.IdServicio == 0 ? 1 == 1 : o.IdServicio == dto.IdServicio)
                                                                                        && (dto.IdSubservicio == 0 ? 1 == 1 : o.IdSubservicio == dto.IdSubservicio)
                                                                                    )
        ...
        public async Task<ActionResult<RespondSearchObject<List<VWListarFacturacionTotal>>>> visualizarMantPrograFacturacion(DTOMantPrograFacturacion dto)//Cambiar este DTO
        {
            var listOportunidad = new List<VWListarFacturacionTotal>();
            ...
            try
            {
                ...
                switch (dto.IdRol)
                {
                    ...
                    //Socio      //que se devolverá
                        listOportunidad = await _context.VWListarFacturacionTotal.Where(o => 
                                                                                  (dto.IdSocio == 0 ? 1 == 1 : o.IdSocio == dto.IdSocio)
                                                                                   && (dto.RazonSocial == "" || dto.RazonSocial == "0" || dto.RazonSocial == null ? 1 == 1 :  o.RazonSocial == dto.RazonSocial)

                                                                                   && (dto.IdServicio == 0 ? 1 == 1 : o.IdServicio == dto.IdServicio)
                                                                                   && (dto.IdGerente == 0 ? 1 == 1 : o.IdGerente == dto.IdGerente)
                                                                                   
                                                                                   && (dto.engagement == "" || dto.engagement == null ||
                                                                                       dto.engagement == "0"  ? 1 == 1 : o.engagement == dto.engagement)

                                                                                   && (dto.id_oportunidad == 0 ? 1 == 1 : o.id == dto.id_oportunidad)
                                                                                   && (dto.IdFacturado == 0 ? 1 == 1 : (dto.IdFacturado == 1 ? o.Fee == o.ImporteProgramado : o.ImporteProgramado < o.Fee))
                                                                                   )
                        .Select(o => new VWListarFacturacionTotal
                         {
                            id = o.id,
                            engagement = o.engagement,
                            IdFacturado = o.ImporteProgramado > 0 ? 2 : 1,
                            IdSocio = o.IdSocio,
                    ...
                    //Cualquier otro rol
                    default:
                        var emptyList = new List<VWListarFacturacionTotal>();
                        listOportunidad = emptyList;
                        break;
                }

                if (listOportunidad.Count > 0)
                    return new RespondSearchObject<List<VWListarFacturacionTotal>>()
                    {
                        Objeto = listOportunidad,
                        Mensaje = "Se encontro Registro",
                        Flag = true
                ...
                //SemiAdministrador
                    ...
                    case 6:
                        ...
                        listOportunidad = await _context.VWListarReporteFacturacion.Where(o => o.FlgActivo == 1
                                                                                   ...
                                                                                   && (dto.IdSocio == 0 ? 1 == 1 : o.IdSocio == dto.IdSocio)
                                                                                   && (dto.IdGerente == 0 ? 1 == 1 : o.IdGerente == dto.IdGerente)
                                                                                   && (dto.id_oportunidad == 0 ? 1 == 1 : o.id_oportunidad.ToString().Contains(dto.id_oportunidad.ToString()))
                                                                                   && (dto.engagement == "0" || dto.engagement == null ? 1 == 1 : o.engagement.Contains(dto.engagement))
