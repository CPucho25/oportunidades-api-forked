    ...
    public class MantenimientoController : ControllerBase
    {
        ...
        {            
            try
            {
                ...
                switch (dto.IdRol)
                {
                    //Administrador
                    case 1:
                        listGerente = _context.VWListarGerente.Where(p => (dto.IdPeriodo == 0 ? p.IdPeriodo == 3 : p.IdPeriodo == dto.IdPeriodo))
                    ...
                    //SemiAdministrador
                    case 6:
                        listGerente = _context.VWListarGerente.Where(p => (dto.IdPeriodo == 0 ? p.IdPeriodo == 3 : p.IdPeriodo == dto.IdPeriodo) && p.IdArea == dto.IdArea)
                        .Select(l => new DTOLista
                        {
                            Id = l.IdGerente,
                            Descripcion = l.Descripcion
        ...
        public RespondSearchObject<List<DTOLista>> listarSocio(DTO.DTOOportunidad dto)
        {
            try
            {
                ...
                switch (dto.IdRol)
                {
                    ...
                    //SemiAdministrador
                    case 6:
                        //listSocio = _context.VWListarSocio.Where(p => (dto.IdPeriodo == 0 ? p.IdPeriodo == 3 : p.IdPeriodo == dto.IdPeriodo) && p.IdArea == dto.IdArea)
                        listSocio = _context.VWListarSocio.Where(p => (dto.IdPeriodo == 0 ? 1 == 1 : p.IdPeriodo == dto.IdPeriodo) && p.IdArea == dto.IdArea)
                        .Select(l => new DTOLista
                        {
                            Id = l.IdSocio,
        ...
        public async Task<RespondSearchObject<DTOReasignar>> reasignarOportunidad(DTO.DTOReasignar dto)
        {
            try
            {
                ...
                #region Capturamos los servicios de la relacion anterior antes de editar(para el caso de que solo se quiera dar de baja algunos servicios)

                var listServ_old = _context.OpRSocGerCli.Where(item => item.IdPeriodo == listaServicio[0].IdPeriodo && item.IdSocio == listaServicio[0].IdSocio
                ...
                foreach (DTORelacionServicio serv in listaServicio)
                {
                    ...
                    switch (serv.Value)
                    {
                        ...
                        //OTROS SERVICIOS
                        default:
                            //Validamos si existen registros previos con las nuevas llaves ingresadas(si no existe nueva relacion será igual a NULL)
                            newRelEntity = _context.OpRSocGerCli.FirstOrDefault(item => item.IdPeriodo == dto.IdPeriodo && item.IdSocio == dto.IdSocio
                                                                               && item.IdGerente == dto.IdGerente && item.IdEmpresa == dto.IdEmpresa
                                                                               && item.IdServicio == serv.Value);

                            //Nuevo servicio que se agrego en la pantalla y gerente nuevo(actualizar registro anterior)
                            if (newRelEntity == null)
                            {

                                //Realizamos la búsqueda del ID a editar en llave anterior(per + emp + serv + ger + soc)
                                entity = _context.OpRSocGerCli.FirstOrDefault(item => item.IdPeriodo == serv.IdPeriodo && item.IdSocio == serv.IdSocio
                                                                               && item.IdGerente == serv.IdGerente && item.IdEmpresa == serv.IdEmpresa
                                                                               && item.IdServicio == serv.Value
                                                                               && item.FlgActivo == 1);
                            ...
                            {//euscuvil 15-11-2022
                                ...
                                if (entity != null)
                                {
                                    #region Si existe Registro Activo, solo informamos que ya existe(no se hace ningun cambio)
                                    ...
                                    */
                                    ...
                                    if (dto.IdEmpresa == listaServicio[0].IdEmpresa && dto.IdSocio == listaServicio[0].IdSocio && dto.IdGerente == listaServicio[0].IdGerente)
                                    {
                                        sgcExist += "Ya existe relación para el siguiente servicio: " + serv.Label + "\n"; //CP ---------> 22.08.23

                                        //boolRegDup = true;

                                    }

                                    #region cambiando el flag a los demás registros pero que tiene otros 'gerentes'(anulado)
                ...
                #region Enlista los servios seleccionados en el formulario y luego desactiva los serevicios de la relacion anterior(Anulado)
                ...
                */ //------> CP
                ...
                #region Evalua los Servicios Originales vs la Lista de servicios final(para detectar los sevicios que fueron eliminados para la misma relacion Emp-Soc-Ger-Serv)
                //Elimina(da de baja) los servicios para el caso que mantiene el Socio-Gerente-------------------------------> CP 21.08.23
                if (dto.IdEmpresa == listaServicio[0].IdEmpresa && dto.IdSocio == listaServicio[0].IdSocio && dto.IdGerente == listaServicio[0].IdGerente) 
                {
                    foreach (OpRSocgercli servOld in listServ_old)
                    {
                        int itemDelete = 0;

                        foreach (DTORelacionServicio servNew in listaServicio)
                        {
                            if (servOld.IdEmpresa == dto.IdEmpresa && servOld.IdSocio == dto.IdSocio && servOld.IdGerente == dto.IdGerente)
                            {
                                if (servOld.IdServicio == servNew.Value)
                                {
                                    //Si existe servicio para dicha relacion pero esta oculta(dado de baja es decir flag = 0) se activa (revisar arriba)
                                    itemDelete += 1;
                                }
                                else
                                {
                        ...
                        if (itemDelete == 0)
                        {
                            var entityOld = _context.OpRSocGerCli.FirstOrDefault(item => item.Id == servOld.Id);

                            //Validate
                            if (entityOld != null)
                            {
                                // Make changes on entity
                                entityOld.FecModificacion = dto.FecModificacion;
                                entityOld.FlgActivo = 0;

                                // Update entity in DbSet Usuario
                                _context.OpRSocGerCli.Update(entityOld);
                                // Save changes in database Usuario
                                _context.SaveChanges();
